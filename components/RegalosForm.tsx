...
export default function RegaloForm() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="font-medium text-gray-700 mb-3">Elige tu regalo:</p>
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200">
            <table className="w-full text-sm">
              <thead className="bg-pink-50">
                <tr>
                  <th className="text-left px-4 py-3 text-pink-600 font-semibold">Regalo</th>
                  <th className="text-right px-4 py-3 text-pink-600 font-semibold">
                    Monto ({moneda === 'BOB' ? 'Bs' : '$'})
                  </th>
                ...
                {opciones.map((opcion, i) => {
                  ...
                  return (
                    <tr
                      ...
                      } ${seleccionada ? '!bg-pink-50 ring-2 ring-inset ring-pink-400' : 'hover:bg-pink-50/50'}`}
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <span className="text-xl">{opcion.emoji}</span>
                          <div>
                            <p className="font-medium text-gray-800">{opcion.nombre}</p>
                            <p className="text-xs text-gray-400">{opcion.descripcion}</p>
                          </div>
                        </div>
                      </td>
      ...
      {opcionSeleccionada && (
        <form onSubmit={handleSubmit} className="space-y-4 pt-2 border-t border-gray-100">
          <div className="bg-pink-50 rounded-lg px-4 py-2 text-sm text-pink-700">
            Seleccionaste:{' '}
            ...
            <>
              {moneda === 'BOB' && opcionSeleccionada.qrUrl && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2">
                  <p className="text-sm font-semibold text-blue-700">Envio de regalo por QR:</p>
                  <p className="text-xs text-blue-600">Escanea el QR para pagar:</p>
              ...
              {moneda === 'USD' && opcionSeleccionada.pagoUrl && (
                <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                  <p className="text-sm font-semibold text-purple-700 mb-3">Enlace de pago:</p>
                  <a
